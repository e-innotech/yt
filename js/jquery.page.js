...
(function($){
	...
	Page.prototype.fillHtml=function(){
		...
		if(args.showPrev){
			if(args.current > 1){
				...
				obj.remove('.prevPage');
				obj.append('<span class="disabled clearfix"><</span>');
			}
		}
		//中间页码
		if(args.current != 1){
		...
		if(args.showNext){
			if(args.current== args.pageCount||args.pageCount==1){
				obj.remove('.nextPage');
				obj.append('<span class="disabled clearfix margin">></span>');
			}else{
	...
	Page.prototype.setStyle=function(){
		...
		this.obj.children().css({'float':'left','text-align':'center'});
		...
		this.obj.find('span.disabled').css({'display':'inline-block','height':s.height+'px','line-height':s.height+'px','color':s.disableColor,'background':s.disableBackColor,'border':'1px solid'+s.borderColor,'vertical-align':'middle','font-size':s.fontSize+'px','width':s.prevNextWidth+'px','border':'1px solid #dedfe3'});
		...
		this.obj.find('.tcdNumber').css({'width':s.pageWidth+'px','height':s.height+'px','line-height':s.height-2+'px'});
